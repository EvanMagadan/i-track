...
export function AdminAlerts({
  ...
  const [sortBy, setSortBy] = useState<
    "daysAsc" | "daysDesc" | "dueDateAsc" | "dueDateDesc" | "monthAsc"
  >("daysAsc");
  ...
  const sortedList = useMemo(() => {
    ...
    return next.sort((a, b) => {
      ...
      if (sortBy === "dueDateDesc")
        return (
          b.dueDate.localeCompare(a.dueDate) || a.name.localeCompare(b.name)
        );
      if (sortBy === "monthAsc") {
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm">
          Proactive billing notifications and overdue account tracking
        </p>
      </div>

      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-6">
        <div className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold border bg-destructive text-destructive-foreground border-destructive">
          <AlertTriangle className="w-4 h-4" />
          Overdue
          <span className="font-mono text-xs px-1.5 py-0.5 rounded bg-white/20">
            {overdue.length}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-5">
        <p className="text-xs text-muted-foreground max-w-xl">
          Sort the current alert list by days overdue, due date, or month.
        </p>
        ...
        <div className="space-y-3 sm:space-y-4">
          {sortedList.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="bg-card border border-red-200 rounded-xl px-4 sm:px-5 py-4 sm:py-5 flex flex-col xl:flex-row xl:items-center justify-between gap-4 sm:gap-5"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between xl:justify-end gap-3 sm:gap-4 shrink-0">
                  <div className="text-left sm:text-right">
                    <p className="font-mono font-bold text-lg sm:text-xl leading-none">
                      {formatCurrency(c.plan)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Due: <span className="font-mono">{c.dueDate}</span>
                    </p>
                  </div>
                  <div className="bg-red-100 text-red-700 rounded-xl px-3 py-2.5 text-center min-w-[76px]">
                    ...
                    <p className="text-xs font-semibold mt-0.5">
                      {days === 1 ? "Day" : "Days"} Past Due
                    </p>
                  </div>
                  <button
                    onClick={() => onRecordPayment(c)}
                    className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-green-600 text-white text-xs font-semibold hover:bg-green-700 transition shrink-0 w-full sm:w-auto"
